(ns bsi_standards.main
  "Kotodama WASM entrypoint for the Bsi_standards clean-room actor (L4) — Clojure port.
  ...
  facts (`bsi_standards.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "bsi_standards")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"bsi_standards.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "bsi_standards-compat" :tier tier :entities entities} 200])
...
(defn start! [] :bsi_standards-compat/ready)
